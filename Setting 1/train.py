    ...
    def validate(self, data='dev', do_test=False):
        ...
        with torch.no_grad():
            for dial_idx, dialog in tqdm(enumerate(eval_data)):
                ...
                """
                if dial_idx == 1:
                    print("[user] ",self.tokenizer.decode(dialog[0]['user']))
                    print("[usdx] ",self.tokenizer.decode(dialog[0]['usdx']))
                    print("[resp] ",self.tokenizer.decode(dialog[0]['resp']))
                    print("[bspn] ",self.tokenizer.decode(dialog[0]['bspn']))
                    print("[bsdx] ",self.tokenizer.decode(dialog[0]['bsdx']))
                    print("[aspn] ",self.tokenizer.decode(dialog[0]['aspn']))
                    print("[dspn] ",self.tokenizer.decode(dialog[0]['dspn']))
                    print("[db] ",self.tokenizer.decode(dialog[0]['db']))
                ...
                for turn_idx, turn in enumerate(dialog):
                    ...
                    print("[user] ",self.tokenizer.decode(turn['user'])) #DEMO
                    ...
                    print(self.tokenizer.decode(turn['resp'])) # DEMO
        ...
        return eval_results
#
    def demo(self, data='dev', do_test=False):
        # path='experiments/all_best_model_sd11_lr0.0001_bs2_ga16/epoch18_trloss0.72_gpt2'
        # python train.py -mode demo -cfg eval_load_path=$path use_true_prev_bspn=False use_true_prev_aspn=False use_true_db_pointer=False use_true_prev_resp=False use_true_curr_bspn=False use_true_curr_aspn=False use_all_previous_context=True cuda_device=0
        # predict one dialog/ one turn at a time
        self.model.eval()
        ...
        with torch.no_grad():
            ...
            turn = {'user': [], 'usdx': [], 
                    ...
                    #'bspn': [self.tokenizer.encode(['<sos_b>'])[0], self.tokenizer.encode(['<eos_b>'])[0]], 'bsdx': [50316, 50309], 
  
                    'dspn': [50318, 50260, 50312], 
                    'pointer': [0, 0, 0, 0, 0, 0], 
                    'turn_domain': ['[taxi]'], 
                    'turn_num': turn_idx, 'db': [50319, 50321, 50320]}

            while True:

                user_utterance = input("Enter an utterance: ")
                if user_utterance == '':
                    break
                user_utterance = self.tokenizer.encode(user_utterance)
                user_utterance_encode_with_tokens =   list(itertools.chain([self.tokenizer.encode(['<sos_u>'])[0]],user_utterance,[self.tokenizer.encode(['<eos_s>'])[0]]))
                turn['user'] = user_utterance_encode_with_tokens
                turn['usdx'] = user_utterance_encode_with_tokens
                ...
                # predict bspn, access db, then generate act and resp ### 通常都用這個（真實！！！！！！！！！！！！）
                outputs = self.model.generate(input_ids=inputs['context_tensor'],
                ...
                inputs['context_tensor_db'] = torch.tensor([inputs['context'][:-1] + bspn_gen + db]).to(self.device)
                context_length = len(inputs['context_tensor_db'][0])
                outputs_db = self.model.generate(input_ids=inputs['context_tensor_db'], max_length=context_length+80, temperature=0.7, pad_token_id=self.tokenizer.eos_token_id, eos_token_id=self.tokenizer.encode(['<eos_r>'])[0])
                generated_ar = outputs_db[0].cpu().numpy().tolist()
                generated_ar = generated_ar[context_length-1:]
                try:
                    decoded = self.decode_generated_act_resp(generated_ar)
                ...
                print("turn_num = ",turn_idx)
                print(self.tokenizer.decode(turn['bspn_gen']))
                print(self.tokenizer.decode(turn['aspn_gen']))
                print(self.tokenizer.decode(turn['resp_gen']))
...
def main():
    ...
    if args.mode == 'test' or args.mode == 'adjust' or args.mode == 'demo':
        ...
        if cfg.exp_path in ['', 'to be generated']:
            ...
            cfg.exp_path = os.path.join(experiments_path,'{}_{}_sd{}_lr{}_bs{}_ga{}_setting1'.format('-'.join(cfg.exp_domains),
                                                                          cfg.exp_no, cfg.seed, cfg.lr, cfg.batch_size,
                                                                          cfg.gradient_accumulation_steps))

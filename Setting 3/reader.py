...
class MultiWozReader(_ReaderBase):
    ...
    def _load_data(self, save_temp=True):
        ...
        if save_temp: # save encoded data
            ...
            if os.path.exists(encoded_file):
                ...
                logging.info("Encoding time: {:.2f} min".format((time.time()-encode_start)/60))
                # save encoded data
    ...
    def _get_encoded_data(self, fn, dial):
        ...
        for idx, t in enumerate(dial['log']):  # tokenize to list of ids
            ...
            enc['turn_pos'] = self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(
                '<sos_p> ' +  str(idx+1) + ' <eos_p>'))
    ...
    def convert_turn_eval(self, turn, pv_turn, first_turn=False):
        ...
        if cfg.use_true_curr_bspn:
            if cfg.use_true_curr_aspn: # only predict resp
                ...
                context_list = ['turn_pos', 'user', 'bspn', 'db'] #4#
                ...
                prompt = '<sos_a>'
        else: # predict bspn aspn resp. db are not predicted. this part tbd. #4#
            context_list = ['turn_pos', 'user']
        ...
        if first_turn:
            ...
            pv_context = pv_turn['labels'] + pv_turn['bspn'] + pv_turn['db']  + pv_turn['resp'] #4#
            # e43 with BABAU
